...
##with gamma=1e4 --> dominates the beam and starts to generate the defocus!
###7*1e-4 --> dominates the surf error
### 5e4 --> dominates surf error
##3*1e4 --> middle grond ?
##1e4 --> dominates beam error
##

#5e3 --> dominates beam map
#1e4 --> domniates surf error


##conjugate
##81e3 --> goes for minimizing the geometry
###1   --> ok (?) 57 of rms


gamma = 10#8*1e3#5*1e4#5*1e7
...
fit_geo_params = '20260505_21286000/images_geo_aperture/geo_fit/params.npz'
...
for i in range(iters):
    ...
    if((i%10) == 0):
        ...
        E_pred, deform_mse = forw_function(params['coeffs'])
        ...
        ax[1,1].imshow(20*np.log10(np.abs(error)), vmin=-80, vmax=10)
