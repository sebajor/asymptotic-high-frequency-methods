...
#fit_geo_params = 'images_geo_beam_conj/geo_fit/params.npz'
fit_geo_params = 'images_geo_aperture_conj/geo_fit/params.npz'
...
def make_forward_function(panels, s_pos0, s_n0, s_ds, sec_vertex,
    ...
    @jax.jit
    def forward_function(coeffs, sec_adjust, sec_rot_adjust):
        sec_offs = sec_offsets+sec_adjust
        sec_rot = sec_rotation+sec_rot_adjust
        s_pos, s_n = secondary_position_update(s_pos0, s_n0, sec_vertex, sec_offs, sec_rot)
...
def loss_funct(params, gold, gamma, gamma2, norm_point=256*128+128):
    pred, deform_mse = forw_function(params['coeffs'], params['sec_adjust'], params['sec_rot_adjust']) 
    ...
    #error = aperture-gold
    loss = jnp.mean(error.real**2+error.imag**2)+gamma*jnp.mean(deform_mse)+gamma2*(
            jnp.linalg.norm(params['sec_adjust'])+jnp.linalg.norm(params['sec_rot_adjust']))
...
params = {
        ...
        "sec_rot_adjust":jnp.ones(3)*1e-10
        }
...
for i in range(iters):
    ...
    print("sec_rot_adjust %.4f, %.4f, %.4f"%(params['sec_rot_adjust'][0]*1e3,params['sec_rot_adjust'][1]*1e3,params['sec_rot_adjust'][2]*1e3) )
    if((i%10) == 0):
        fig, ax = plt.subplots(2,2,figsize=(10,10))
        plot_deformations(panels, params['coeffs'],ax=ax[0,0], correct_global=0)
        E_pred, deform_mse = forw_function(params['coeffs'], params['sec_adjust'], params['sec_rot_adjust'])
        ...
        optic_error = np.linalg.norm(params['sec_adjust'])+np.linalg.norm(params['sec_rot_adjust'])
        ...
        title += "optics error: %E"%optic_error
