...
for i in range(len(test_r_points)):
    for j in range(len(test_t_points_p)):
        for k in range(len(test_t_points_s)):
            ...
            print("r_points: %i t_prim: %i t_sec: %i"%(r_points, t_points_primary, t_points_secondary))
            ...
            sec_offset_j = jnp.array([x.to_value(apu.m) for x in sec_offset]).astype(jnp.float32)
            s_pos_j = jnp.array(s_pos.to_value(apu.m)).astype(jnp.float32)
            ...
            target_pos_j = jnp.array(target_pos.to_value(apu.m)).astype(jnp.float32)
            ...
            s_n_j = jnp.array(s_n).astype(jnp.float32)
            s_ds_j = jnp.array(s_ds.to_value(apu.m**2)).astype(jnp.float32)
            p_pos, p_n, p_ds, panel_ms = apply_panel_deformation(panels, coeffs)

            horn_position_j = (jnp.array((0,0,B.to_value(apu.m))).T).astype(jnp.float32)
            ...
            def make_forward_function(panels, s_pos0, s_n, s_ds,
                                ...
                                target_pos, horn_position, edge_tapper, horn_aperture,
                                 wavel, batch_size):
                @jax.jit
                def forward_function(sec_offset):
            ...
            forw_function = make_forward_function(panels, s_pos_j, s_n_j, s_ds_j,
                    p_pos, p_n, p_ds,
                     target_pos_j, horn_position_j, edge_tapper.to_value(apu.dB),
                     horn_aperture.to_value(apu.m), wavel.to_value(apu.m),
                     batch_size)
            E = forw_function(sec_offset_j)
